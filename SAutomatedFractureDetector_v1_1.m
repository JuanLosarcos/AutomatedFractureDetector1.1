...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Title: SAutomatedFractureDetector_v1_1
...
%load the  normalized edge image
pridgeFile = fullfile(output_folder, ['P_Ridges_' InFileListShort{1}]);

% Read the normalized edge strength image from the previous step
C_Ridges = imread(pridgeFile); %should be read the previous ridge picture?
...
%
% Diff vs. original:
%   – Georeferencing/shapefile export (commented).
...
%% Choose axis tick step (world units) with an interactive loop

% pixel size in world units per pixel
pixelSize = scaleLength / pixelDist;   % (scaleUnits)/px

% Load image once
img = imread(InFileList{1});
[rows, cols, ~] = size(img);

% Allow up to 10 attempts to choose the step
for attempt = 1:10
    close all
    fAxis = figure('Name','Axis Step Check','NumberTitle','off');
    imshow(img); 
    axis on; axis image;

    % Ask user for integer step in world units
    stepWorld = input(['\nEnter the axis tick step in m' (numeric only, e.g., 1, 2, 5, 10): ']);

    % World extents
    xWorldMax = cols * pixelSize;
    yWorldMax = rows * pixelSize;

    % World tick labels (0, step, 2*step, ...)
    xTicksWorld = 0:stepWorld:floor(xWorldMax);
    yTicksWorld = 0:stepWorld:floor(yWorldMax);

    % Convert world ticks to pixel positions
    ax = gca;
    ax.XTick = xTicksWorld / pixelSize;
    ax.YTick = yTicksWorld / pixelSize;

    % Label ticks in world units
    ax.XTickLabel = string(xTicksWorld);
    ax.YTickLabel = string(yTicksWorld);

    xlabel(['X (' scaleUnits ')'])
    ylabel(['Y (' scaleUnits ')'])
    title('Axis tick step preview (no fractures)')

    % Ask if user wants to adjust again
    redo = input('Adjust axis step? [Yes/No]: ','s');
    if any(strcmpi(redo, {'No','N'}))
        close(fAxis);
        break
    end
...
n = length(PolyLines_Rotated{1,1});
hLine = gobjects(n,1);

for i = 1:n
    xy_px = PolyLines_Rotated{1,1}{i,1};
    % clickable line (instead of scatter)
    hLine(i) = plot(xy_px(:,1), xy_px(:,2), '-', 'Color','r', 'LineWidth', 1);

    % store fracture ID so datacursor knows which one you clicked
    hLine(i).UserData = i;
    hLine(i).HitTest = 'on';
    hLine(i).PickableParts = 'all';
end
...
% Integer world ticks (0,1,2,3,...)
xTicksWorld = 0:stepWorld:floor(xWorldMax);
yTicksWorld = 0:stepWorld:floor(yWorldMax);

% Convert world tick positions back to pixels (tick *positions* must be in pixels)
ax.XTick = xTicksWorld / pixelSize;
ax.YTick = yTicksWorld / pixelSize;

% Label them as integers
ax.XTickLabel = string(xTicksWorld);
ax.YTickLabel = string(yTicksWorld);
...
% Enable datacursor and customize what it displays
dcm = datacursormode(gcf);
dcm.Enable = 'on';
dcm.UpdateFcn = @(obj,event) fractureTipNoGeo(obj,event,Orient_corr,Length_real,scaleUnits);

fFract = figure(4);
FractGraph = char(outfolder + "Fractures.jpg");
saveas(fFract, FractGraph);

function txt = fractureTipNoGeo(~, event, Orient_corr, Length_real, scaleUnits)
    idx = event.Target.UserData;  % fracture ID

    txt = {
        ['Fracture ID: ' num2str(idx)]
        ['Orient (deg, 0-180): ' num2str(Orient_corr(idx), '%.2f')]
        ['Length (' scaleUnits '): ' num2str(Length_real(idx), '%.4f')]
    };
end
...
%fprintf('Max X shown on axis: %.6f\n', (ax.XLim(2))*pixelSize);
...
minLen  = min(Length_real(Length_real>0));
...
% Pick top 5 bins by summed length
K = 15;
